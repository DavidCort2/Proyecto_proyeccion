...
def render_results(instructors: pd.DataFrame, execution: dict) -> None:
    ...
    with st.expander("Reglas y metodología de esta ejecución"):
        ...
        if execution.get("distribution_basis") == "quarterly_v1":
            st.caption(f"Año de cuatro trimestres con {rules['weeks_per_quarter']} semanas efectivas cada uno. Ingresos al inicio y terminaciones al final del período. " + ("La duración de cada programa y jornada se toma de su malla." if curricular else "O&P/P&O es diurna de 10 trimestres; Técnico regular 3, Tecnólogo diurno 7 y mixto 9."))
